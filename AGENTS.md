...
This file is the working guide for agents editing `huggingface-repos`.

## What this repo is

`huggingface-repos` is a single-page static catalog of public repositories from
Hugging Face's `huggingface` GitHub organization. It follows a
...
- `cloudflare/worker.js`: proxy used for
  `https://glaubi.net/huggingface`.

## Catalog rules

Included repositories must be:

- Owned by the `huggingface` GitHub org.
- Non-forks and non-archived.
- More than 200 stars.
- Pushed within the last three calendar months.

The page contains seven rough ecosystem clusters:

1. Models & Core Libraries
2. Agents, RL & Alignment
3. Training, Inference & Optimization
4. Data, Evaluation & Hub Infrastructure
5. Robotics, Vision, Audio & Multimodal
6. Apps, Web & Developer Tools
7. Education, Courses & Community
...
That command queries GitHub for current `huggingface` repositories
matching the catalog rules, fetches metadata and recent commit counts, assigns
each repo to a rough ecosystem cluster, rebuilds the fresh-traction table,
